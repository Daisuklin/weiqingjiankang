          ...
          }
          {
            showCollectionCart && <Flex.Item style={{ flex: 1, textAlign: 'center' }} onClick={() => this._gotoCart()}>
              <div>
                ...
                <img src={`./assets/img/weiqing/dibu-04@2x.png`} style={{ width: '44px', height: '44px' }} alt="" />
                <div>购物车</div>
              </div>
            </Flex.Item>
          }
          <Flex.Item className='addCart' onClick={()=>this._addCart()} style={{flex:2}}>
            <Button disabled={this.props.data==0?true:false} type='primary' style={{background:'#00a9e0',borderColor:'#00a9e0',height:'0.7rem',lineHeight:'0.7rem',borderRadius:'3px'}}>添加购物车</Button>
          </Flex.Item>
          <Flex.Item className='goBuy' onClick={()=>this._gotoBuy()} style={{flex:2}}>
            <Button disabled={this.props.data==0?true:false} type='primary' style={{background:'#e70012',borderColor:'#e70012',height:'0.7rem',lineHeight:'0.7rem',borderRadius:'3px'}}>立即购买</Button>
          </Flex.Item>
        </Flex>
      </div>
    );
