...
class GoodsDetail extends Component {
    ...
    refresh = () => {
        ...
        bargainApi.bargargainDetail({
            ...
            if (bargargainDetail.shopBargain) {
                let shopBargain = bargargainDetail.shopBargain;
                this.setState({
                    isJoin: true,
                    ...
                    init: true,
                    mowTime: result.now,
                    bargainingNumber: shopBargain.bargainingNumber
    ...
    helpBargain = (bargargainDetail, bargainInfo,goodsDetailInfo) => {
        const self=this;
        BaseApi.getToken({
            ...
            if (result.result == 1) {
                ...
                wx.ready(function () {
                    ...
                    let url= common.SERVER_PATH + '/bargain.html#/helpBargainDetail/' + self.props.params.id + '/' + bargainInfo.bargainId;
                    //分享到朋友圈
    ...
    render() {
        ...
        return (
            <div className='wx-goods-detail-bargain'>
                <div ref='detailScroll' className='fix-scroll hastitle hasbottom' style={{
                    backgroundColor: 'white', position: 'relative',paddingTop:'0rem',
                    height: `${document.documentElement.clientHeight/parseFloat(document.getElementsByTagName('html')[0].style.fontSize)}rem`,
                }}>
                        ...
                        <Carousel autoplay={false} infinite={false} dots={true} selectedIndex={0} className="imgBoxdiv">
                            {
                                ...
                                item!=""?<Img key={index} src={item} onClick={this.showModal('modal1', item)}/>:null
                                ))
                            }
                        </Carousel>
                    ...
                    {bargargainDetail.isOpen != 1 ? <div style={{
                        ...
                        <WhiteSpace size="md" style={{background: '#f3f3f3'}}/>
                        {
                            this.state.isJoin ? <Flex style={{padding: '0.25rem 0.2rem'}}>
                                <Flex.Item style={{color: '#666666', fontSize: '0.28rem', lineHeight: '0.45rem'}}>
                                    已经有{bargainingNumber}位亲友帮忙砍价，共砍掉<span
                                    style={{color: '#e50414'}}>￥{parseFloat(bargainInfo.cutTotalPrice).toFixed(2)}</span>,想以最低价收入囊中。那就赶紧找亲友拔刀相助吧！
                                </Flex.Item>
                        ...
                        <BargainMoreInfo goodsDetailInfo={goodsDetailInfo} bargainId={bargainInfo.bargainId}
                                         barbarianDetail={bargargainDetail}></BargainMoreInfo>
                    </div>}
                    {this.state.isJoin ?
                        <Flex className="bargainBtn">
                            <Flex.Item style={{flex: 1}}>
                                {
                                    bargargainDetail.isOpen != 1 ? '' : <Button type='primary' onClick={() => {
                                        this.helpBargain(bargargainDetail, bargainInfo,goodsDetailInfo)
                                    }} inline style={{
                                        height: '0.65rem',
                                        width: '42%',
                                        margin: '0rem 0.2rem',
                                ...
                                {
                                    bargargainDetail.isOpen != 1 ? <Button type='primary'
                                                                           onClick={() => {
                                                                               this.props.router.push('/bargainPurchase/' + bargainInfo.bargainId)
                                                                           }}
                                                                           disabled
                                                                           style={{
                                                                               height: '0.65rem',
                                                                               ...
                                                                               // borderColor: '#e70012',
                                                                               borderRadius: '5px'
                                                                           }}>立即购买</Button> : <Button type='primary' inline
                                                                                                      onClick={() => {
                                                                                                          this.props.router.push('/bargainPurchase/' + bargainInfo.bargainId)
                                                                                                      }}
                                                                                                      style={{
