...
export default function({ goodsDetailInfo }) {
  ...
  return <Tabs animated={false} defaultActiveKey="1" swipeable={false} activeTextColor="#E43F47" className="goodsMoreInfo">
    <TabPane tab="商品介绍" key="1">
      {
        goodsBody && goodsBody.length > 0 && goodsBody.map((item,index) => {
          if (item.type == 'image') {
            return <div key={index} style={{fontSize:'0rem'}}><Img  src={item.value} style={{width:'100%'}}></Img></div>
          }
        })
      }
    </TabPane>
