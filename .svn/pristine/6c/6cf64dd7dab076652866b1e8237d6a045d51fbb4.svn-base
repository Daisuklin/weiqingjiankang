...
class ApplyAfterSale extends Component {
  constructor(props) {
    ...
    componentDidMount() {
        const orderid = this.props.location.state.orderItem.orderId;
    ...
    if (selectedAction == 1) {
      ...
      orderApi.refundOrder({
        ...
        if (r.result == 1) {
          Toast.info(r.msg);
          //Toast.hide();
          this.props.router.replace('/ginsegOrderList/0')
        } else {
          Toast.info(r.msg);
        }
      })
      ...
      orderApi.barterOrder({
        ...
        if (r.result == 1) {
          Toast.info(r.msg);
            //Toast.hide();
          this.props.router.push('/ginsegOrderList/0')
        } else {
          Toast.info(r.msg);
        }
      })
  ...
  submit = () => {
    const {  goodsItem, type } = this.props.location.state;
    const { files, selectedAction, buyerMessage,orderItem, goodsNum } = this.state;
    if (buyerMessage == '') {
