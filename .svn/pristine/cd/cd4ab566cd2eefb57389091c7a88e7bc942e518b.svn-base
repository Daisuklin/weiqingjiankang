...
class Order extends Component {
    ...
    onClickSelectedAddress = () => {
        //debugger
        window.location.href='groupBuy.html#/address';
    ...
    refresh = () => {
        ...
        const buyCount = this.props.params.groupNumber;
        const groupDetailId = this.props.params.groupDetailId;
        genen.grouppurchase({
            specId:specId,
            ...
            if (result.result == 1){
                const spellInfo = result.data;
                console.log(spellInfo)
                if (spellInfo.addressList && spellInfo.addressList.length > 0) {
                    this.setState({
                        spellInfo: spellInfo,
                        init: true
                    });
        ...
        return <div className='wx-order'>
            <div className='fix-scroll hastitle' style={{paddingBottom:'1.1rem'}}>
                <List>
                    ...
                    <Item onClick={this.onClickSelectedAddress}
                     ...
                     style={{background:'url(./assets/img/weiqing/dizhi-02@2x.png) 0px bottom / 100% 8px no-repeat'}}>
                        {
                            addressList!=null? <div style={{padding:'0.2rem 0px'}}>
                                <Flex justify="between" >
                                    <Flex.Item style={{flex:0.2}}>
                                        <img src="./assets/img/weiqing/dizhi-01@2x.png" style={{width:'25px',height:'35px'}}/>
                                    </Flex.Item>
                                    <Flex.Item style={{flex: 3, marginLeft: '0px'}}>
                                        <span>{addressList[0].trueName}{' '}{addressList[0].mobPhone}</span>
                                        <Brief>{addressList[0].areaInfo } {' '} {addressList[0].address}</Brief>
                                    </Flex.Item>
                                </Flex>
                            </div> :'请选择地址'
                        }
                ...
                <div className="orderlist"><Item
                    ...
                    <Flex style={{padding:'0.2rem 0px'}} justify="between">
                        <Flex.Item style={{flex:1,maxWidth:'1.62rem',marginRight:'0.2rem'}}>
                            <Img src={spellInfo.groupImage} style={{height:'1.3rem',width:'1.62rem'}}/>
                        </Flex.Item>
                        <Flex.Item style={{flex:3}}>
                            <div style={{width:'100%'}}>
                                <div className="goodsname">{spellInfo.groupName}</div>
                                {/*<div className="goodsproduct">{data.groupName}</div>*/}
                                <div style={{overflow:'hidden'}}>
                                    <Brief style={{fontSize:'0.3rem',color:'#e60012',float:'left'}}>¥{spellInfo.groupPrice}</Brief>
                                    <Brief style={{color:'red',fontSize:'0.24rem',color:'#333',float:'right'}}>x{spellInfo.goodsNumber}</Brief>
                                </div>
                ...
                <Flex direction="column" >
                    <Flex.Item style={{borderRight: '1px solid #ddd',width:'100%'}}>
                        <List className="extrared">
                            <Item extra={`¥${spellInfo.goodsAmount}`}>商品总价</Item>
                            <Item extra={`+ ¥${spellInfo.shippingFee}`}>运费</Item>
                        </List>
                    </Flex.Item>
                    <WhiteSpace size="md" style={{background:'#f3f3f3'}}/>
                    <Flex.Item style={{ background:'#fff',padding:'0.3rem 0.26rem',width:'100%',marginLeft:'0px'}}>
                        <div className="order_all">
                            <div className="order_all_price" onClick={()=>console.log(spellInfo.orderAmount)}>合计： <span style={{fontSize:'0.28rem',color:'#e2536b'}}>{`¥${spellInfo.orderAmount}`}</span></div>
             ...
             <SpellOrderBar
                    ...
                    specId={spellInfo.specId}
                    groupItemId={spellInfo.groupItemId}
                    shippingFee={spellInfo.shippingFee}
                    groupDetailId={spellInfo.groupDetailId}
                    shareUrl={common.IMAGE_DOMAIN+spellInfo.groupImage}
                    onSubmitOrder={this.onSubmitOrder} totalPrice={spellInfo.orderAmount} buyCount={spellInfo.goodsNumber}>
                </SpellOrderBar>
        </div>
    }
}
