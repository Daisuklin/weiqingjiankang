...
class GoodsDetail extends Component {
  ...
  addCart = (count) => {
    ...
    cartApi.addCart({
      ...
      } else if(result.result == 0 && result.errorCode != 403){
        Toast.fail(result.msg);
      }
    });
  }
  ...
  render() {
    ...
    return (
      <div className='wx-goods-detail'>
        ...
        <Carousel autoplay={false} infinite={false} dots={true} selectedIndex={0} style={{height:'7.5rem'}}>
          {
            goodsDetailInfo.goodsCallyList.map((item,index) => {
                return <Img key={index} src={item} onClick={this.showModal('modal1',item)} style={{height:'7.5rem'}}/>
            })
          }          
        </Carousel>
         <Modal title="商品详情"
          ...
          <Flex className='wx-goods-detail-info'  justify="between" align='center' >
            <Flex.Item style={{flex:3}}>
              ...
              <p style={{margin:'10px 0px 15px'}}><span style={{color:'#e82231',fontSize:'.3rem',paddingRight:'0.2rem'}}>{`¥${goodsDetailInfo.qiangPrice ? goodsDetailInfo.qiangPrice:goodsDetailInfo.goodsSpec.specGoodsPrice}`}</span><del style={{color:'#9a9a9a',fontSize:'.28rem'}}>{`¥${goodsDetailInfo.qiangPrice ? goodsDetailInfo.goodsSpec.specGoodsPrice:goodsDetailInfo.goodsSpec.specMarketPrice}`}</del></p>
            </Flex.Item>
              {
                  goodsDetailInfo.geneGoods==1 && <Flex.Item style={{flex:1,textAlign:'center',color:'#656565'}} onClick={()=>{window.open(common.IMAGE_DOMAIN + goodsDetailInfo.geneUrl)}}>
                    <img src='./assets/img/weiqing/xiazai@2x.png' style={{width:'0.5rem',height:'0.5rem',paddingBottom:'0.1rem'}}/><br /><span>附件下载</span>
                  </Flex.Item>
              }
            {/*<Flex.Item style={{flex:1,textAlign:'center',color:'#656565'}} onClick={()=>{window.open('http://dl.download.csdn.net/down11/20170516/51b39faf4ed6833d8fe6d85d99920639.pdf?response-content-disposition=attachment%3Bfilename%3D%22%E6%B7%B1%E5%85%A5REACT%E6%8A%80%E6%9C%AF%E6%A0%88%20%2C%E9%99%88%E5%B1%B9%E8%91%97%20%2CP351%20%2C2016.11%20%20%E5%89%AF%E6%9C%AC.pdf%22&OSSAccessKeyId=9q6nvzoJGowBj4q1&Expires=1498558955&Signature=R3ucaDBKLn15PG46LbnTe6whDrU%3D')}}>
              <img src='./assets/img/weiqing/xiazai@2x.png' style={{width:'0.5rem',height:'0.5rem',paddingBottom:'0.1rem'}}/><br /><span>附件下载</span>
            </Flex.Item>*/}
          </Flex>
        ...
        <List className="goodsDetail_condition">
          ...
          <List.Item>
            运费：{goodsDetailInfo.goodsTransfeeCharge==1?'卖家承担运费':goodsDetailInfo.goodsTransfeeCharge==2?'免运费':'买家承担运费'}
          </List.Item>
        </List>
                  <WhiteSpace size="md" style={{background:'#f3f3f3'}}/>
        <EvaluateGoodsList
