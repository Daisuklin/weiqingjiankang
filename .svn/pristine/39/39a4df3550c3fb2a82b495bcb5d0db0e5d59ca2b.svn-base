...
class BargainSpec extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      goodsSpecValueAll: props.goodsDetailInfo.goodsSpecValueAll,
        BargainSpec: props.goodsDetailInfo.BargainSpec,
  ...
  renderHeader = () => {
    const { goodsImage, BargainSpec, goodsName } = this.state;
    return <div>
      ...
      <Flex style={{ height: '200px' }}>  
        ...
        <Flex.Item style={{flex:2}}>
          <div style={{color:'red'}}>{`¥${BargainSpec.specGoodsPrice}`}</div>
          <div>{goodsName}</div>
        </Flex.Item> 
      </Flex>
    </div>
  ...
  onChangeSpec = (spec) => {
    const { BargainSpec, goodsSpecValueAll } = this.state;
    // 当前选择的所有规则
    let currentSpecs = BargainSpec.specGoodsSpec;
    // 删除当前规则组的所有子选项
  ...
  render() {
    ...
    const {
      goodsSpecValueAll, // 所有的规格属性
        BargainSpec, // 当前选择的规格值
      specName,
      goodsName,
      goodsImage,
    } = this.state;
    // 当前选中的规格
    const { specGoodsSpec } = BargainSpec;
      ...
      <List renderHeader={() => (this.renderHeader())}>
        ...
        <List.Item extra={
          <Stepper ref='stepper'
            ...
            showNumber min={0}
            max={parseInt(BargainSpec.specGoodsStorage)}
            size="small"
            onChange={this.onChangeNum}
            value={this.state.buyCount} />
          }>数量</List.Item>
        <List.Item>库存:{BargainSpec.specGoodsStorage}</List.Item>
