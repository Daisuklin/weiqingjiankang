...
class Store extends Component {
    ...
    onClassification = (stateNum) => {
        const {params} = this.props;
        console.log(stateNum);
        this.props.router.push({
            pathname: `/store/${params.storeId}/goods`,
            state: {
                stateNum
            }
        })
        ...
        return <div className='wx-store'>
                                          ...
                                          style={{height: '2.05rem'}}>
                                    {
                                        arr.map((imgSrc, index) => {
                                            if(imgSrc!=' '&&imgSrc!=null){
                                                return <Img key={index} src={imgSrc} style={{height:'2.22rem'}}/>
                                            }
            ...
            <div className='wx-store-bar'>
                <Flex style={{width: '100%', textAlign: 'center'}}>
                    ...
                    <Flex.Item onClick={() => this.onClassification(1) }>
                        热门分类
                    </Flex.Item>
